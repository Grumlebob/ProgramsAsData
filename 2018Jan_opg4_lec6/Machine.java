...
class Machine {
  ...
  static void execute(String[] args, boolean trace)
    ...
    int[] instructionsCounter = new int[26]; // Variables
    for (int i = 1; i < args.length; i++) // Push commandline arguments
      iargs[i - 1] = Integer.parseInt(args[i]);
    long starttime = System.currentTimeMillis();
    execcode(p, s, iargs, trace, instructionsCounter); // Execute program proper
    long runtime = System.currentTimeMillis() - starttime;
    System.err.println("\nRan " + runtime / 1000.0 + " seconds");
    printByteCodeStat(instructionsCounter);
  }

  // The machine: execute the code starting at p[pc]

  static int execcode(int[] p, int[] s, int[] iargs, boolean trace, int[] instructionsCounter) {
    ...
    for (;;) {
      ...
      var currentInstruction = p[pc];
      instructionsCounter[currentInstruction] += 1;
      ...
      switch (p[pc++]) {
        ...
        case LDARGS:
          ...
          break;
        case STOP:
          // print all entries of instructionsCounter
          for (int i = 0; i < instructionsCounter.length; i++) {
            System.out.println("Instruction " + i + " was executed " + instructionsCounter[i] + " times");
          }
          return sp;
        default:
  ...
  static String insnameSimple(int instruction) {
    switch (instruction) {
      case CSTI:
        return "CSTI";
      case ADD:
  ...
  static void printByteCodeStat(int[] byteCodeStat) {
    System.out.println("ByteCodeStatistics:");
    for (int i = 0; i < byteCodeStat.length; i++) {
      System.out.println(insnameSimple(i) + " : " + byteCodeStat[i]);
    }
  }
