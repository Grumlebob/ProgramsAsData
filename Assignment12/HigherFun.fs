...
let rec evalExpr (env: value env) (e: expr<typ>) (cont: value -> answer) (econt: value -> answer) : answer =
    ...
    | TryWith(e1, ExnVar exn, e2) ->
        evalExpr env e1 cont (fun vExn1 ->
...
and evalValdecs
    ...
    | Fundecs fs :: ts' ->
        ...
        let env' =
            (List.map (fun (f, closRef) -> (f, ClosureRef closRef)) closureRefs) @ env

        evalValdecs env' ts' body cont econt
    | Valdec(x, eRhs) :: ts' -> evalExpr env eRhs (fun xVal -> evalValdecs ((x, xVal) :: env) ts' body cont econt) econt
