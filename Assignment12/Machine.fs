...
type instr =
    ...
    | HEAPLDI of
        int (* LDI operation on heap allocated object / Load pejer plus offset N( hive variable ud af closure=. *)
...
let resetLabels, newLabelWName =
    ...
    ((fun () -> lastlab := 0),
     (fun name ->
         (lastlab := 1 + !lastlab
          (if name = "" then "" else name + "_") + "L" + (!lastlab).ToString())))
...
let ppInst (addr, strs) instr =
    let indent s =
        (addr + sizeInst instr, "  " + (addr.ToString().PadLeft(4)) + ": " + s :: strs)
