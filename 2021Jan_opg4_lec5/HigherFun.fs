...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Prim(ope, e1, e2) -> 
      ...
      | ("*", Double d1, Double d2) -> Double (d1 * d2) //exam
      | ("+", Int i1, Int i2) -> Int (i1 + i2)
      | ("+", Double d1, Double d2) -> Double (d1 + d2) //exam
      | ("-", Int i1, Int i2) -> Int (i1 - i2)
      | ("-", Double d1, Double d2) -> Double (d1 - d2) //exam
      | ("=", Int i1, Int i2) -> Int (if i1 = i2 then 1 else 0)
      | ("=", Double d1, Double d2) -> Int (if d1 = d2 then 1 else 0) //exam
      | ("<", Int i1, Int i2) -> Int (if i1 < i2 then 1 else 0)
      | ("<", Double d1, Double d2) -> Int (if d1 < d2 then 1 else 0) //exam
    ...
    | Prim1(ope, e1) -> 
      let v1 = eval e1 env
      match (ope, v1) with
      | ("toDouble", Int i1) -> Double (double i1) //exam
      | ("toInt", Double d1) -> Int (int d1) //exam
      | _ -> failwith "unknown primitive or wrong type"
    | Let(x, eRhs, letBody) -> 
