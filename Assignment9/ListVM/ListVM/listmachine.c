...
#define Length(hdr)   (((hdr)>>2)&0x003FFFFF) //finds word size. Removes first two bits, then returns the next 22 bits (size)

#define Color(hdr)    ((hdr)&3) //returns the first two bits (color)
#define Paint(hdr, color)  (((hdr)&(~3))|(color)) // &~3 sets the color to 0. |color sets the new color value, while keeping remaining bits intact
...
void printInstruction(word p[], word pc)
{
    switch (p[pc])
    {
    case CSTI: printf("CSTI " WORD_FMT, p[pc + 1]);
        break;
    case ADD: printf("ADD");
        break;
    case SUB: printf("SUB");
    ...
    case STI: printf("STI");
        break;
    case GETBP: printf("GETBP");
        break;
    case GETSP: printf("GETSP");
        break;
    case INCSP: printf("INCSP " WORD_FMT, p[pc + 1]);
        break;
    case GOTO: printf("GOTO " WORD_FMT, p[pc + 1]);
        break;
    case IFZERO: printf("IFZERO " WORD_FMT, p[pc + 1]);
        break;
    case IFNZRO: printf("IFNZRO " WORD_FMT, p[pc + 1]);
        break;
    ...
    case TCALL: printf("TCALL " WORD_FMT " " WORD_FMT " " WORD_FMT,
        ...
        break;
    case RET: printf("RET " WORD_FMT, p[pc + 1]);
        break;
    case PRINTI: printf("PRINTI");
        break;
    case PRINTC: printf("PRINTC");
        break;
    case LDARGS: printf("LDARGS");
        break;
    ...
    case CDR: printf("CDR");
        break;
    case SETCAR: printf("SETCAR");
        break;
    case SETCDR: printf("SETCDR");
        break;
...
void heapStatistics()
{
    ...
    printf(
        "Heap: " WORD_FMT " blocks (" WORD_FMT " words); of which " WORD_FMT " free (" WORD_FMT " words, largest "
        WORD_FMT " words); " WORD_FMT " orphans\n",
...
int isPointerToHeap(word v)
{
    return IsInt(v) ? 0 : v != 0 && inHeap((word*)v);
}
...
void mark(word* block)
{
    ...
    {
        //dont check index 0 as it is the head.
        if (isPointerToHeap(block[i]))
        {
            mark((word*)block[i]); //take value as pointer and recursively mark whatever it points to
        }
    }
}
...
{
    //exc.30 should only mark one (xs) as black and the rest should be sweeped
    printf("marking ...\n");

    while (sp > 0)
    {
        word v = s[sp--];
        if (isPointerToHeap(v))
        {
            mark((word*)v);
        }
    }
