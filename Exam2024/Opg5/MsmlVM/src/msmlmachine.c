...
void printInstruction(word p[], word pc) {
  ...
  case PRINTSTAT: printf("PRINTSTAT"); break; //exam 2024
...
Number of stack frames: 2
*/ 
void printStat(word topElementOnStack, word s[], word bp, word sp) {
  printf("-------PRINTSTAT " WORD_FMT "-------\n", Untag(topElementOnStack)); //untag fordi det er en int value
  printf(" Size stack: " WORD_FMT "\n", sp+1); //sp pejer på toppen af stacken, og derfor er også vores stack size.
  word currentBasePointer = bp; 
  word numFrames = 0;
  while (currentBasePointer != -999) {
    numFrames++; //Vi så ikke bunden af stacken, aka når bp er -999 fordi lige bagefter kommer globals osv. så vi taller en frame op.
    currentBasePointer = Untag(s[currentBasePointer - 1]); //Det fordi  at bp peger på første locals, og lige under første local er der old basepointer.
    }
