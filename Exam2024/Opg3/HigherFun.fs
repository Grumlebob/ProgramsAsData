...
type value = 
  ...
  | QueueV of value list (* Exam 2024 *)
...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Prim(ope, e1, e2) -> 
      ...
      | ("<", Int i1, Int i2) -> Int (if i1 < i2 then 1 else 0)
      | ("++", QueueV q1, QueueV q2) -> QueueV (q1 @ q2) // Exam 2024 - Union two queues
      | ("++", Int q1, QueueV q2) -> QueueV ( Int q1 :: q2) // Exam 2024 - Union element to queue
      | ("++", QueueV q1, Int q2) -> QueueV ( q1 @ [Int q2]) // Exam 2024 - Union queue to element
      | ("->>", QueueV q1, QueueV q2) -> QueueV (q1 @ q2) //Exam 2024 - Insert Tail queue to queue
      | ("->>", Int q1, QueueV q2) -> QueueV ( Int q1 :: q2) //Exam 2024 - Insert Tail element to queue
      | ("->>", QueueV q1, Int q2) -> QueueV ( q1 @ [Int q2]) //Exam 2024 - Insert Tail queue to element
      |  _ -> failwith "unknown primitive or wrong type"
    | Prim1(s, e1) -> //Exam 2024 - Return Head (last index of list
      let v1 = eval e1 env
      match (s, v1) with
      | ("<<-", QueueV q1) -> 
        match q1 with
        | [] -> failwith "empty queue"
        | xs -> List.rev xs |> List.head //Head of queue is last index of list
      | _ -> failwith "unknown primitive or wrong type"
    | Queue(list) -> QueueV (List.map (fun x -> eval x env) list) //Exam 2024 - Queue
