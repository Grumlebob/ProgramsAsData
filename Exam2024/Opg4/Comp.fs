...
let rec allocate (kind : int -> var) (typ, x) (varEnv : varEnv) : varEnv * instr list =
    ...
    | TypA (t, Some i) ->
      ...
      (newEnv, code)
    | TypR listOfFields -> //Exam 2024. Vi allocator ikke selve record, men alle dens felter
        let rec allocateFields (fields : (typ * string) list) (environmentAccumulator : varEnv) : varEnv * instr list =
            match fields with
            | [] -> environmentAccumulator, []
            | (fieldTyp, fieldName)::restOfList ->
                let fullFieldName = x + "." + fieldName //We don't want "x" we want "coords.x"
                let (updatedEnvironment, updatedInstructionlist) = allocate Locvar (fieldTyp, fullFieldName) environmentAccumulator //They are all local variables
                let (completeEnviroment, completeInstructionslist) = allocateFields restOfList updatedEnvironment //Recursive call on rest of fields
                (completeEnviroment, updatedInstructionlist @ completeInstructionslist) //concat the result
        let (newEnv, code) = allocateFields listOfFields varEnv //start loop with all fields
        (newEnv, code) //return the result
    | _ -> 
