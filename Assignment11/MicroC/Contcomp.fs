...
let rec addIFZERO lab3 C : instr list =
    match C with
    | GOTO lab2 :: Label lab3 :: C1 -> IFNZRO lab2 :: Label lab3 :: C1
    | _ -> IFZERO lab3 :: C
...
let rec addCST i C =
    match (i, C) with
    | (vari,  CSTI topvariable2 :: LT :: restofCode) ->
        if vari < topvariable2 then
            addCST 1 restofCode
        else
            addCST 0 restofCode
    | (number11, CSTI number22 :: SWAP :: LT :: restofCode) ->
        if number11 > number22 then
            addCST 1 restofCode
        else
            addCST 0 restofCode
    | (number11, CSTI number22 :: EQ :: restofCode) ->
        if number11 = number22 then
            addCST 1 restofCode
        else
            addCST 0 restofCode
...
and cExpr (e: expr) (varEnv: varEnv) (funEnv: funEnv) (C: instr list) : instr list =
    ...
    | Cond(expr, expr1, expr2) ->
        let (jumpend, C1) = makeJump C
        let (labelse, C2) = addLabel (cExpr expr2 varEnv funEnv C1)

        cExpr expr varEnv funEnv (addIFZERO labelse (cExpr expr1 varEnv funEnv (addJump jumpend C2)))
    | Orelse(e1, e2) ->
        match C with
